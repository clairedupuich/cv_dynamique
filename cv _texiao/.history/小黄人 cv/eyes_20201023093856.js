...
function eyeball() {
  ...
  eye.forEach(function(eye) {
    ...
    let angle = (radian * (180 / Math.PI) * -1) - 20;// 计算角度 //* 270是调整眼睛的角度，即鼠标在下方时要保证眼珠也在下面，-1是调整眼睛转动的方向要于鼠标游走方向一致。
    /*PI一般用来代表圆周率π
     角度 = 弧度 * 180 / Math.PI;
    弧度= 角度 * Math.PI / 180; */
    // 修改旋转角度
...
txt2.className = 'texAfter texAfter2';
txt2.textContent="One Piece,Dragon Ball Z, Gintama et Le Pacte des Yōkai.Chaque anime est un monde indépendant. Ils sont si loin de nous, mais ils nous rappellent de temps en temps de garder notre innocence et notre passion pour la vie .";
txt2.addEventListener("mouseover", change);
function change(){

    image2.replaceChild(txt2,image2.firstChild);
}; 
/* 获取转动的角度
