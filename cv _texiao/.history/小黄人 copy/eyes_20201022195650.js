...
function eyeball() {
  ...
  eye.forEach(function(eye) {
    ...
    let x = (eye.getBoundingClientRect().left + (eye.clientLeft/2));//我认为 除2或者不除2都会一样运行，甚至不加边框也是一样的，不同是对于之后使用的函数来说更加严谨，这样眼珠就不会太靠近眼眶边缘
    // 同上
    let y = (eye.getBoundingClientRect().top + (eye.clientTop /2/* / 2 */));
